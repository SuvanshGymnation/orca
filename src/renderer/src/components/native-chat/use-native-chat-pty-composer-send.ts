...
}): (queuedText?: string) => void {
  // A queued drain supplies its own text and must not touch the live draft:
  // the user may be typing the next message while the queue empties.
  return useCallback(
    (queuedText?: string) => {
      const isQueuedSend = queuedText !== undefined
      const text = queuedText ?? args.draft
      const imagePaths = isQueuedSend
        ? []
        : args.imageAttachments.map((attachment) => attachment.path)
